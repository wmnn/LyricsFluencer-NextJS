import React, {useState, useContext} from 'react'
import {auth} from '../../src/util/firebase'
import { createUserWithEmailAndPassword } from "firebase/auth";
import UserContext from '../Context/UserContext.jsx'
import { useRouter } from "next/router";
import Button from '../Button';
import Input from '../Input';

function RegisterModal({setIsRegisterClicked, setIsLoginClicked}) {
  const router = useRouter()
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const {userContext, setUserContext} = useContext(UserContext);
    
    const handleSignUp = (e) => {
      e.preventDefault();
      createUserWithEmailAndPassword(auth, email, password)
      .then((userCredential) => {
        console.log(userCredential)
        setUserContext(userCredential)
        router.push(`/onboarding/plans?plan=premium`)
        
      }).catch((error) => {
        console.log(error)
      })
    }

    
  
  return (
    <div className="fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center text-xl">
      <div className='flex flex-col mt-[-10%] w-[80%] md:w-[40%]'>
        <div className='flex justify-end'>
          <button className='text-2xl hover:cursor-pointer text-red-600' onClick={() => setIsRegisterClicked((prev) => !prev)}>X</button>
        </div>
        <div className='bg-gray-200 w-[100%] rounded-xl shadow-2xl flex justify-center'>
          <form /*onSubmit={handleSignUp} */className="flex flex-col w-[80%] my-8">
            
              <label className='flex flex-col justify-between'>
                <p>Email:</p>
                <Input handleChange={(e) => setEmail(e.target.value)} type={"email"} value={email}/>
                {/* <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-gray-100 rounded-xl p-2"/> */}
              </label>

            <label className='flex flex-col justify-between my-2'>
              Password:
              <Input handleChange={(e) => setPassword(e.target.value)} type={"password"} value={password}/>
            </label>

            <div className='mb-4'/>

            <Button 
              text={"Create Account"}
              color={"bg-black"}
              handleClick={() => handleSignUp(event)}
            />
      
            <div className='flex justify-center mt-4'>
            <span className='inline-flex'>Already have an account? <p className="pl-2 hover:cursor-pointer" onClick={() => {
              setIsRegisterClicked((prev) => !prev)
              setIsLoginClicked((prev) => !prev)
            }}>Login</p></span>
            </div>
          </form>
          
        </div>
      </div>
      

    </div>
    
  )
}

export default RegisterModal